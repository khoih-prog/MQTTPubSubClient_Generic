...
static lwmqtt_err_t lwmqtt_drain_network(lwmqtt_client_t *client, size_t amount)
{
  ...
  while (amount > 0)
  {
    ...
    // read
    size_t partial_read = 0;
    lwmqtt_err_t err = client->network_read(client->network, client->read_buf, max_read,
                                            &partial_read, (uint32_t)remaining_time);
...
static lwmqtt_err_t lwmqtt_read_packet_in_buffer(lwmqtt_client_t *client, size_t *read, lwmqtt_packet_type_t *packet_type)
{
  // preset packet type
                            ...
                            uint32_t timeout) 
{
  ...
  size_t len = 0;
  lwmqtt_err_t err = lwmqtt_encode_publish(client->write_buf, client->write_buf_size, &len, 0, packet_id, topic, message);
